...
public class FloatResource extends MQTTResource implements LocalResource {

	private static final Logger LOG = LoggerFactory.getLogger(FloatResource.class);
	...
	public FloatResource(String name, boolean bWrite, float value) {
		super(name);
		this.bWrite = bWrite;
		this.value = value;
	}
	 ...
	 */
	public void setValue(float currentTemp) {
	...
	public void handlePOST(MQTTExchange exchange) {
		...
		if(this.bWrite){
			exchange.respond(ResponseCode.CONTENT, Float.toString(this.value));
		} else {
